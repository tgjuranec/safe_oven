					...
					CHIP_IOCON_PIO_T e, CHIP_IOCON_PIO_T lcdpwr, CHIP_IOCON_PIO_T lcdled)
{
	...
	lc.lcdled = lcdled;
	lc.lcdpwr = lcdpwr;
	//when lcd_counter is '0' it starts printing
	//onto the LCD counting up to '34', when it's '34' it's in quiet state
	lcd_counter = 34;
...
void LCD1602_exec(){
	...
	if(lcd_counter < 34){
		if(lcd_counter == 0){
			LCD1602_print_ctrl(LCD_1602_CLEAR_DISPLAY);
		}
...
void LCD1602_print(const char *line1, const char *line2){
	//checking whether the last transmission is closed
	if(lcd_counter >= 34){
		//copy first line
		uint8_t i=0;
